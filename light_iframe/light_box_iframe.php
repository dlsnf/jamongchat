...
$get_toppx = $_GET['toppx'];
		...
		$query="SELECT boo.*, uoo.seq uoo_seq, uoo.name uoo_name, uoo.email uoo_email, uoo.profile_way uoo_profile_way, uoo.profile uoo_profile FROM jamong_chat_freeboard boo LEFT JOIN jamong_chat_user uoo ON boo.user_seq = uoo.seq WHERE boo.seq = $get_seq"; // SQL 쿼리문
...
<div style="">
			...
			<div class="light_box" style="">
				...
				<div class="light_box_group">
					...
					<div class="text_div content_board">
						...
						<img class="profile_img light_box_info_img" src='<?=$board['profile_way']?>' style="background:white;"  width='40px' height="40px" onload="tooltip_fun(); finish_date(<?=$board['seq']?>);" onclick="parent.profile_light_iframe_start(<?=$board['user_seq']?>);" title="프로필 보기">

							<div class="light_box_info_info">
							
								<div class="user_name mouse_over_underline" style="font-weight: bold; width:150px; text-overflow: ellipsis; white-space: nowrap;overflow: hidden;"><?=$board['name']?></div><div class="date_time"><?=$board['year']?>년 <?=$board['month']?>월<?=$board['day']?>일 <?=$board['ampm']?> <?=$board['hour']?>:<?=$board['minute']?></div>
							...
							?>
													<div class="comment_write comment_write_<?=$board['seq']?>">
														...
														<form id="myForm_<?=$board['seq']?>" name="myForm_<?=$board['seq']?>" action="../comment_write_ajax.php" method="post" accept-charset="utf-8" ENCTYPE="multipart/form-data">
															<img class="profile_img" src="<?=$user_profile_way?>" style="background:white;" width="30px" height="30px" onclick="parent.profile_light_iframe_start(<?=$user_seq?>);" title="프로필 보기">
