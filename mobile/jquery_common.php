...
function menu_off()
{
	...
	setTimeout(function(){
		...
		$(".btn_menu_close").removeClass("btn_menu_close_hide");
	},500);
	...
	switch(url)
	{
		...
		case "/jamongchat/mobile/board.php": 
				...
				if( board_type == 'free' )
				{
					...
					$("#search").attr("placeholder","자유게시판 - 검색");
				}else if(board_type == 'fashion' )
				{
					$(".navigate span").text("자몽챗 >> 패션");	
					$("#search").attr("placeholder","패션 - 검색");
				}else if(board_type == 'selfie' )
				{
					$(".navigate span").text("자몽챗 >> 셀피");	
					$("#search").attr("placeholder","셀피 - 검색");
				}else if(board_type == 'food' )
				{
					$(".navigate span").text("자몽챗 >> 음식");	
					$("#search").attr("placeholder","음식 - 검색");
				}else if(board_type == 'photo' )
				{
					$(".navigate span").text("자몽챗 >> 사진");	
					$("#search").attr("placeholder","사진 - 검색");
				}
