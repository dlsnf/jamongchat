...
<div class="hot_issue_div hot_issue_<?=$hot_board['seq']?>">
		<ul class="finish_date finish_date_<?=$hot_board['seq']?> tooltip2" title="게시글 폭파까지 남은시간" style="right:0px; top:0px;">
		...
		<div class="info" style="padding:0px; margin-bottom:10px;">
			...
			<span><div class="user_name mouse_over_underline" style="display: inline-block; font-weight:bold; width:90px; text-overflow: ellipsis; white-space: nowrap;overflow: hidden;"><?=$hot_board['name']?></div><br>
			<div class="date_time">
<?
...
?>
		...
		<div class="hot_photo_div"  title="자세히 보기" style="margin-bottom:14px;">
			...
			<a href="detail_content.php?seq=<?=$hot_board['seq']?>&board=<?=$board_type2?>&mobile=<?=$mobile_check2?>" title="해당 게시물로 이동">
				<div class="hot_photo_div2_<?=$hot_board['seq']?>" style="width:230px; height:150px; overflow:hidden;">
					<img class="hot_con_img_<?=$hot_board['seq']?>" src="<?=$hot_board['photo_way']?>thumbnail/<?=$hot_board['photo']?>" style="  position: relative;" width="230px" onload="hot_issue_img_load(<?=$hot_board['seq']?>);">
				</div>
			</a>
			
			<div class="hot_like" style="margin-top:2px;padding-top:2px;">
